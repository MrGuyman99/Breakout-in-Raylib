...
Blocks::Blocks(const char *imagePath, Vector2 imagePosition){

    BlockPosition = imagePosition;
    BlockTexture = LoadTexture(imagePath);
...
void Blocks::Update(Ball& ball){
    ...
    if(Destroyed == false){
        ...
        if(CheckCollisionCircleRec(Vector2{ball.BallPosition.x + 24, ball.BallPosition.y + 24}, 8, 
